...
<aside style="background-color:  #FF8AAE;"
    ...
    <div class="" id="sidenav-collapse-main">
        ...
        <ul class="navbar-nav">
            ...
            <li class="barra">
                <a class="nav-link text-white active"  style="background: #9ADCFF;" href="{{ url('outputs') }}">
                    <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
                        <i class="material-icons opacity-10">table_view</i>
                    </div>
            ...
            <div class="card my-4" style="background: #9ADCFF;">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class=" shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-content-center"
                        ...
                        <h3 class="text-white  text-capitalize ps-3"> <i>S a l i d a s</i></h3>
                        <form class="d-inline" action="{{ url('/outputs') }}"
                            method="post">
                            @csrf
                            ...
                            <tbody>
                                ...
                                @foreach ($outputs as $output)
                                    <tr class="tabla text-center">
                                        <td>
                                            <span
                                                class="text-secondary text-xs font-weight-bold">{{ $output->id }}</span>
                                        </td>
                                        <td class="align-middle align-content-center  ">
                                            <form class="d-inline" action="{{ url('/outputs/' . $output->id) }}"
                                                method="post">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                                <button class="text-secondary font-weidght-bol text-xs enlace btn barra"
