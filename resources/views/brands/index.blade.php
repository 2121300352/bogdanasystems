            ...
            <div class="card my-4" style="background: #9ADCFF;">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class=" shadow-primary border-radius-lg pt-4 pb-3 d-flex justify-content-between align-content-center"
                        ...
                        <h3 class="text-white  text-capitalize ps-3"> <i>M a r c a s</i></h3> <a style="height: 50px"
                            class="btn barra" href="{{ url('brands/create') }}"> <img style="height: 100%"
                            ...
                            <tbody>
                                ...
                                @foreach ($brands as $brand)
                                    <tr class="tabla text-center">
                                        ...
                                        <td>
                                            <span
                                                class="text-secondary text-xs font-weight-bold">{{ $brand->id }}</span>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            <span
                                                class="text-secondary text-xs font-weight-bold">{{ $brand->name }}</span>
                                        </td>
                                        <td class="align-middle text-center text-sm">
                                            <span
                                                class="text-secondary text-xs font-weight-bold">{{ $brand->description }}</span>
                                        </td>
                                        <td class="align-middle align-content-center  ">
                                            <a class="text-secondary font-weidght-bol text-xs enlace btn barra"
                                                href="{{ url('/brands/' . $brand->id . '/edit') }}">
                                                <img style="height: 20px; opacity: .6"
                                                        src="{{ asset('img/editar.png') }}" alt="">
                                                </a>
                                            <form class="d-inline" action="{{ url('/brands/' . $brand->id) }}"
                                                method="post">
                                                @csrf
                                                {{ method_field('DELETE') }}
                                                <button
